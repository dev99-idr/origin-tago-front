	...
	var tree = {
			...
			if (this.rendered) {
				if (p_parentNode==undefined) {
					...
					if (p_parentNode.childNodes.length==0) {
						...
						v_img.src = '../../resource/assets/js/lib/aimaraJS/images/collapse.png';
						v_img.id = 'toggle_off';
						}
						else {
							...
							v_img = p_parentNode.elementLi.getElementsByTagName("img")[0];
							v_img.style.visibility = "visible";
							v_img.src = '../../resource/assets/js/lib/aimaraJS/images/expand.png';
							v_img.id = 'toggle_on';
						}
					}
		...
		drawNode: function(p_ulElement,p_node) {
			...
			if (p_node.childNodes.length == 0) {
				v_exp_col = createImgElement('toggle_off','exp_col','../../resource/assets/js/lib/aimaraJS/images/collapse.png');
				...
				if (p_node.expanded) {
					...
					v_exp_col = createImgElement('toggle_on','exp_col','../../resource/assets/js/lib/aimaraJS/images/expand.png');
				}
			}
			...
			if (v_icon!=undefined)
				...
				if(p_node.parent!=null){
					v_a = createSimpleElement('a','cnt§'+p_node.parent.text.trim()+"§"+p_node.text.trim(),null);
					v_a.style.fontSize = '15px';
				}
				else{
					v_a = createSimpleElement('a','cnt§null§'+p_node.text.trim(),null);
					v_a.style.fontSize = '15px';
				}
		...
		expandNode: function(p_node) {
			if (p_node.childNodes.length>0 && p_node.expanded==false) {
				...
				img.id="toggle_off";
				img.src = '../../resource/assets/js/lib/aimaraJS/images/collapse.png';
			...
			if(document.getElementById('cntArrowleft_'+$("#ul_"+p_node.id).siblings('span').children('a').attr('id').split('§')[2])!=undefined){
                document.getElementById('cntArrowleft_'+$("#ul_"+p_node.id).siblings('span').children('a').attr('id').split('§')[2]).style.display='';
                document.getElementById('cntArrowright_'+$("#ul_"+p_node.id).siblings('span').children('a').attr('id').split('§')[2]).style.display='';	
            }
		},
		///// Collapsing node
		// p_node: Reference to the node;
		collapseNode: function(p_node) {
			if (p_node.childNodes.length>0 && p_node.expanded==true) {
				...
				img.id="toggle_on";
				img.src = '../../resource/assets/js/lib/aimaraJS/images/expand.png';
				elem_ul = img.parentElement.getElementsByTagName("ul")[0];
				elem_ul.style.display = 'none';

			}
			if(document.getElementById('cntArrowleft_'+$("#ul_"+p_node.id).siblings('span').children('a').attr('id').split('§')[2])!=undefined){
                document.getElementById('cntArrowleft_'+$("#ul_"+p_node.id).siblings('span').children('a').attr('id').split('§')[2]).style.display='none';
                document.getElementById('cntArrowright_'+$("#ul_"+p_node.id).siblings('span').children('a').attr('id').split('§')[2]).style.display='none';	
            }
		},
		///// Toggling node
		// p_node: Reference to the node;
				...
				if (p_node.contextMenu!=undefined) {
					...
					for (var i=0; i<v_menu.elements.length; i++) (function(i){
						...
						if (v_menu.elements[i].submenu!=undefined) {
							...
							v_span_more.appendChild(createImgElement(null,'menu_img','../../resource/assets/js/lib/aimaraJS/images/right.png'));
